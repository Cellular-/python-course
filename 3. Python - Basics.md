...
`if`, `elif` and `else` are used to decide if a part of your program should run or not. Control flow will start with `if` and can "branch" off to other parts of your program based on some expressions created with [[3. Python - Basics#Logical operators]|logical operators]]. `if` and `elif` are followed by expressions that evaluate to either `True` or `False`.
...
`and` is used to combine multiple expressions and will evaluate to `True` only if all of the conditional statements evaluate to `True`. Let's examine the `if` statement above:
